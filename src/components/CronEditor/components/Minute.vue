<template>
  <div>
    <RadioGroup v-model:value="type" @change="valueChange">
      <Radio v-for="list in MinuteTypeList" :value="list.value" :key="list.value">
        {{ list.label }}
      </Radio>
    </RadioGroup>
    ...
    <div class="typeSettring">
      <div class="setting-items mt-2" v-if="type == 'ZQ'">
        ...
        <InputNumber class="w160" :min="2" :max="59" v-model:value="zqEnd" @change="valueChange"></InputNumber> 分
      </div>
      <div class="setting-items mt-2" v-if="type == 'XH'">
        从 <InputNumber class="w160" :min="0" :max="59" v-model:value="xhStart" @change="valueChange"></InputNumber> 分开始，每 
        <InputNumber class="w160" :min="1" :max="59" v-model:value="xhEnd" @change="valueChange"></InputNumber> 分执行一次
      </div>
...
<script lang="ts">
  ...
  import { MinuteTypeList, arr60 } from './data';
